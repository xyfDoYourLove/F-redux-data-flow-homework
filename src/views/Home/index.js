...
class Home extends Component {
  render() {
    return (
      <div className="main-view home">
        <h2>Practice</h2>
        <p>
          用户的登录/登出功能是网站的常见功能之一，请基于该repo及课程所学内容对其进行登录/登出功能的添加
        </p>
        <p>
          登录接口请使用<code>GET</code>请求
          <code>https://my-json-server.typicode.com/kevindongzg/demo/login</code>{' '}
          进行模拟登录用户信息获取如下：
        </p>
        <Highlight>
          {`{
  ...
  "permissions": [ // 用户有哪些模块的权限
    ...
    "NOTIFICATION" // 通知中心
        ...
        <ol>
          <li>登出状态如下图：</li>
          <img src={signOutImg} alt="登出状态" />
          <li>
            点击 <code>Sign in</code> 通过登录接口进行登录（点击请求接口直接登录），进入登录状态。
          </li>
          <li>登录状态如下图：</li>
          <img src={signInImg} alt="登录状态" />
          <li>
          ...
          <li>
            除了<code>首页</code>
            外，登录状态下菜单栏的展示需要根据用户各模块权限信息进行展示，即"报表"菜单在该用户登录后不应该展示。
          </li>
          <li>
